...
<table id="country-list" border="0">
    ...
    <tr class="trs">
        ...
        <td class="tds"><a href=""
                class="liste"><?php echo textWrap($product["doc_fullnames"], 60) . '  ' . $product['doc_serialcode']; ?></a>
            <?php
                        if (isset($_SESSION['clientIdLost'])) {
                        ?>
            <br>
            <button class="ClaimButton"><a href="claim?doc=<?php echo actor($product['doc_id']) ?>"
                    style="color:#fff !important"> Claim Document</a< /button>
                    <?php } else { ?>
                    <button class="ClaimButton"><a href="login" style="color:#fff !important">Login to Claim Document
                            </a< /button>
                            <?php } ?>
